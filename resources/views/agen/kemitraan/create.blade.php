...
<div class="max-w-4xl mx-auto pt-3 md:pt-5 pb-12 px-3 md:px-6">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 px-4 py-6 md:p-8 shadow-sm mb-8" data-aos="fade-up">
        <div class="relative flex items-center justify-between mb-10">
            ...
            <div class="relative z-10 flex flex-col items-center">
                <div class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-bold text-sm {{ $currentStep > $index ? 'bg-[#58CC02] text-white' : ($currentStep == $index + 1 ? 'bg-white border-4 border-[#58CC02] text-[#58CC02]' : 'bg-white border-4 border-gray-100 text-gray-300') }}">
                ...
                <span class="absolute top-10 md:top-12 text-[9px] md:text-[10px] font-bold uppercase tracking-wider whitespace-nowrap {{ $currentStep >= $index + 1 ? 'text-[#58CC02]' : 'text-gray-400' }}">{{ $label }}</span>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mb-6 text-center" data-aos="fade-up" data-aos-delay="100">
        <h1 class="text-xl md:text-2xl font-bold text-gray-800">Lengkapi Dokumen Kemitraan</h1>
        <p class="text-gray-500 text-sm mt-1">Unggah file MOU dalam format PDF untuk diverifikasi oleh admin.</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
        {{-- Sidebar Ketentuan --}}
        <div class="md:col-span-1 order-2 md:order-1" data-aos="fade-right" data-aos-delay="150">
            <div class="bg-white rounded-3xl border border-gray-100 p-5 shadow-sm">
                ...
                <ul class="space-y-3 text-xs text-gray-500">
                    <li class="flex gap-2 items-start">
                        ...
                        <span>File wajib berformat PDF (Maks. 10MB).</span>
                    </li>
                    <li class="flex gap-2 items-start">
                        <i class="fa-solid fa-circle-check text-green-500 mt-0.5 shrink-0"></i>
                    ...
                    <li class="flex gap-2 items-start">
                        <i class="fa-solid fa-circle-check text-green-500 mt-0.5 shrink-0"></i>
                        <span>Pastikan data diri dan tanda tangan sudah lengkap.</span>
                    </li>
                </ul>
                <div class="mt-5 pt-5 border-t border-gray-50">
                    <a href="https://docs.google.com/document/d/1l4LvWVaSuImZOfhnV_KPClyWzXpBd3Qy9Ask56TmjCI/export?format=docx" download class="flex items-center justify-center gap-2 w-full py-3 bg-gray-50 text-gray-600 text-xs font-bold rounded-xl border border-gray-100 hover:bg-gray-100 transition-all">
                        <i class="fa-solid fa-download"></i> Download Template MOU
                    </a>
                </div>
            </div>
        </div>
        ...
        {{-- Form Upload --}}
        <div class="md:col-span-2 order-1 md:order-2" data-aos="fade-left" data-aos-delay="150">
            <form id="formUploadMou" action="{{ route('kemitraan.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-gray-100 p-5 md:p-8 shadow-sm">
                @csrf
                <div id="dropzone" class="border-2 border-dashed border-gray-200 rounded-2xl p-6 md:p-10 text-center hover:border-[#58CC02] transition-all group relative">
                    ...
                    <div id="uploadPlaceholder" class="relative z-10 pointer-events-none">
                        <div class="w-14 h-14 md:w-16 md:h-16 bg-gray-50 rounded-2xl flex items-center justify-center mx-auto mb-3 group-hover:bg-green-50 transition-colors">
                            <i class="fa-solid fa-file-pdf text-2xl text-gray-400 group-hover:text-[#58CC02]"></i>
                        </div>
                        <p class="text-gray-600 font-bold text-sm md:text-base mb-1">Pilih atau Seret File PDF</p>
                        <p class="text-gray-400 text-xs">Tap di area ini untuk memilih file dari perangkat Anda</p>
                    </div>

                    <div id="previewContainer" class="hidden relative z-30">
                        <div class="flex items-center justify-between bg-green-50 p-3 md:p-4 rounded-2xl border border-green-100 mb-3">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-9 h-9 md:w-10 md:h-10 bg-white rounded-lg flex items-center justify-center text-[#58CC02] shadow-sm shrink-0">
                                    <i class="fa-solid fa-file-lines text-lg"></i>
                                </div>
                                <div class="text-left min-w-0">
                                    <p id="previewName" class="text-xs md:text-sm font-bold text-gray-800 truncate max-w-[160px] md:max-w-[240px]">document.pdf</p>
                                    <p id="previewSize" class="text-[10px] text-gray-400">0 KB</p>
                                </div>
                            </div>
                            <button type="button" id="btnRemoveFile" class="text-gray-400 hover:text-red-500 transition-colors cursor-pointer border-none bg-transparent shrink-0 ml-2">
                ...
                <div id="errorMessage" class="hidden mt-4 p-3 md:p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 text-xs md:text-sm">
                ...
                <div id="actionContainer" class="hidden mt-6">
                    <button type="button" onclick="openModal('uploadModal')" class="w-full py-3.5 md:py-4 bg-[#58CC02] text-white font-black rounded-2xl shadow-lg shadow-green-100 uppercase tracking-widest hover:opacity-90 transition-all cursor-pointer border-none text-sm">
